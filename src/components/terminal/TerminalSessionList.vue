...
async function load() {
  const res = await apiGet<{ sessions?: Session[] }>("/api/terminal/sessions");
  if (res.ok) {
    sessions.value = res.data.sessions ?? [];
    error.value = null;
  } else if (sessions.value.length === 0) {
    // Only surface the error when we have nothing to show yet — load()
    // runs on every pub/sub push and a transient failure must not replace
    // an already-populated list with an error banner.
...
onMounted(() => {
  load();
  unsubscribe = subscribe(PUBSUB_CHANNELS.terminalSessions, () => load());
});
