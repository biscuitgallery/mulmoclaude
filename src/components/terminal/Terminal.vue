...
function connect() {
  // Tear down any existing connection. Its handlers are neutralised below
  // by the `sock !== ws` guards once `ws` is reassigned, so a late event
  // from the old socket can't flip the status or leak output.
  if (ws) ws.close();
  term.reset();
